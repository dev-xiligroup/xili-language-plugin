...
class Xili_Language_Uninstall {
	 ...
	 * @since 2.17
	 */
	public function uninstall( $blog_id = 1 ) {
		// check if delete_settings is set
		global $wpdb;
		$xl_settings = get_option( 'xili_language_settings' );

		if ( isset( $xl_settings['delete_settings'] ) && 'delete' == $xl_settings['delete_settings'] ) {
			// only if instance in multisite is checked
	...
	/*
 	 ...
 	 * @since 1.8.8
 	*/
	public function delete_taxonomies( $xl_settings ) {

		// temporary register taxonomies (plugin is deactivated)

		register_taxonomy(
			$xl_settings['taxonomy'],
			'post',
			array(
				'hierarchical' => false,
				'label' => false,
		...
		foreach ( $languages as $language ) {
			...
			foreach ( $post_ids as $post_id ) {
				...
				foreach ( $postmeta_suffixes as $postmeta_suffix ) {
					if ( $language->slug != $postmeta_suffix ) {
						delete_post_meta( $post_id, $xl_settings['reqtag'] . '-' . $postmeta_suffix );
					}
				}
			...
			if ( $linklang ) {
				wp_delete_term( $term_id, 'link_' . $xl_settings['taxonomy'] );
			}
