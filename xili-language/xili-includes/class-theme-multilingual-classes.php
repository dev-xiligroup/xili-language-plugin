...
if ( ! class_exists( 'xili_language_theme_options' ) ) {
	...
	class Xili_Language_Theme_Options {
		...
		public $capability;
		public $menu_lang_sep; // used when nav menu slug contain _
		public $class_version = '1.5';
		public $child_suffix = '-xili';

		public function __construct( $xili_args ) {
		...
		public function get_theme_xili_options() {
			return get_option( $this->settings_name, $this->get_default_theme_xili_options() );
		}

		public function get_default_theme_xili_options() {

			return array(
				'no_flags' => '',
				'linked_posts' => '1',
				'linked_title' => 'Read this post in',
				'nav_menus' => 'no',
			);
		}

		// create options in customize theme screen to be refreshed
		public function customize_registering( $wp_customize ) {
			...
			if ( $num_locations >= 1 && count( $menu_locations ) >= 1 ) {
				...
				if ( 0 == $xili_language->has_languages_list_menu( $location_slugs[0] ) ) { // only test one menu during transition
					...
					$wp_customize->add_setting(
						'xili_language_settings[in_nav_menu]',
						array(
							'default'     => '',
							'transport'   => 'refresh',
							'type'        => 'option',
					...
					$wp_customize->add_setting(
						'xili_language_settings[nav_menu_separator]',
						array(
							'default'    => '|',
							'transport'  => 'refresh',
							'type'       => 'option',
			...
			$wp_customize->add_setting(
				$this->settings_name . '[linked_posts]',
				array(
					'default'     => '',
					'transport'   => 'refresh',
					'type'        => 'option',
		 ...
		 * @updated 1.0.2
		 */
		public function xili_create_menu_locations() {

			$xili_theme_options = $this->get_theme_xili_options();

			if ( isset( $xili_theme_options['nav_menus'] ) && 'nav_menus' === $xili_theme_options['nav_menus'] ) {
				// ok for automatic insertion of one menu per lang...
				...
				if ( $menu_locations_keys ) {
					foreach ( $menu_locations_keys as $oneloc ) {
						foreach ( $language_slugs_list as $slug ) {
							...
							$do_it = ( 1 == $navmenu_count ) ? true : ( isset( $xili_theme_options[ $indice ] ) && 'nav_menu' == $xili_theme_options[ $indice ] );
							if ( $do_it && $slug != $default ) {
								/* translators: */
								register_nav_menu( $one_menu_location, sprintf( xl__( '%1$s for %2$s', $this->theme_domain ), $menu_locations[ $oneloc ], $slug ) );
							}
						}
					}
				}
		 ...
		 * @since 0.9.7
		 */
		public function xili_wp_nav_menu_args( $args ) {

			$xili_theme_options = get_theme_xili_options();
			$ok = ( isset( $xili_theme_options['nav_menus'] ) && 'nav_menus' == $xili_theme_options['nav_menus'] ) ? true : false;
		 ...
		 *
		 */
		public function xili_clone_sidebar_container() {
					...
					foreach ( $wp_registered_sidebars as $one_key => $one_sidebar ) {
						...
						if ( false === strpos( $one_key, '_' . $slug ) && isset( $xili_theme_options[ $indice ] ) ) {
							...
							register_sidebar(
								array(
									/* translators: */
									'name' => sprintf( xl__( '%1$s in %2$s', $this->theme_domain ), $one_sidebar['name'], $language->description ),
		...
		public function get_processed_default_headers() {
		...
		public function child_installation_msg( $xl_required_version, $minimum_xl_version, $class_ok ) {
			...
			if ( false === $class_ok ) {
				...
				<div class="error">' .
				/* translators: */
					'<p>' . sprintf( esc_html__( 'The %s child theme requires <em>xili_language_theme_options</em> class to set multilingual features.', 'xili-language' ), get_option( 'current_theme' ) ) . '</p>
				</div>';
				...
				<div class="updated">' .
				/* translators: */
					'<p>' . sprintf( esc_html__( 'The %s child theme was successfully activated with xili-language.', 'xili-language' ), get_option( 'current_theme' ) ) . '</p>
				</div>';
				...
				<div class="error">' .
				/* translators: */
					'<p>' . sprintf( esc_html__( 'The %1$s child theme requires xili-language version %2$s+', 'xili-language' ), get_option( 'current_theme' ), $minimum_xl_version ) . '</p>
				</div>';
			}
...
if ( ! class_exists( 'xili_language_theme_options_admin' ) ) {
	...
	class Xili_Language_Theme_Options_Admin extends Xili_Language_Theme_Options {
		...
		public $customize_adds;
		public $capability;
		public $xili_theme_page; // set in menu creation - used by help
		public $admin_ui_domain = 'xili-language';


		public function __construct( $xili_admin_args ) {
		...
		public function xili_options_theme_menu() {
			/* translators: */
		...
		public function xili_options_theme_page() {
			$message = '';
			$indice = ( 'xili-language' == $this->admin_ui_domain ) ? 'X' : 'T';
		?>
		<div class="section panel">
		<h1><?php /* translators: */printf( xl_esc_html__( 'Multilingual options for %1$s%2$s theme ', $this->admin_ui_domain ), $this->theme_name, $this->child_suffix ); ?></h1>
		<?php
		...
		<p><em><?php xl_esc_html_e( 'Click on line with arrow to reveal options to set... Don’t forget to save after changing...', $this->admin_ui_domain ); ?></em></p>
		<p class="submit">
		...
		<p><small><?php echo $this->theme_domain . $this->child_suffix; ?> v. <?php echo $this->child_version; ?> , a multilingual child by <a href="http://dev.xiligroup.com" target="_blank" >dev.xiligroup.com</a> (©2019) <?php echo '(xl v.'.XILILANGUAGE_VER . ') (' . $this->class_version . '.' . $indice . ')'; ?></small></p>
		 ...
		 */
		public function xili_register_settings() {
			...
			foreach ( $menu_locations as $one_key => $one_location ) {
				if ( false === strpos( $one_key, $this->menu_lang_sep ) ) {
					$navmenu_count ++; // only core nav menu
				}
			}
			...
			add_settings_section( 'xili_option_section_5', '<hr />' . xl__( 'Header image options', $this->admin_ui_domain ), array( $this, 'xili_display_one_section' ), $this->settings_name );

			$field_args = array(
				'option_name' => $this->settings_name,
				'title'       => xl_esc_html__( 'Multilingual header', 'xili-language' ),
				'type'      => 'checkbox',
				'id'        => 'xl_header',
				'name'      => 'xl_header',
				'desc'      => xl_esc_html__( 'If checked, one header per language can be selected', 'xili-language' ),
				'std'       => '1', // like via customizer
				'label_for' => 'xl_header',
				'class'     => 'css_class',
			);
			...
			add_settings_section( 'xili_option_section_6', xl__( 'Header image assignment', $this->admin_ui_domain ), array( $this, 'xili_display_one_section' ), $this->settings_name );
			...
			if ( 0 == $section ) {
				...
				$field_args = array(
					'option_name' => $this->settings_name,
					'title' => xl__( 'Message:', $this->admin_ui_domain ),
					'type' => 'message',
					'id' => 'no_header_image',
			...
			if ( $xili_language->is_permalink && version_compare( $language_xili_settings['version'], '2.17', '>=' ) ) {
				...
				add_settings_section( 'xili_option_section_7', '<hr />' . xl__( 'Permalinks for languages', $this->admin_ui_domain ), array( $this, 'xili_display_one_section' ), $this->settings_name );
				$field_args = array(
					'option_name' => $this->settings_name,
					'title'     => xl_esc_html__( 'Activate Permalinks', $this->admin_ui_domain ),
					'type'      => 'message',
					'id'        => 'perma_ok',
					'desc'      => sprintf( xl_esc_html__( 'Now, language in permalink is set in language settings page %s', $this->admin_ui_domain ), '<a href="options-general.php?page=language_expert" >' . xl__( 'here', 'xili-language' ) . '</a>' ),
				...
				if ( file_exists( $xili_functionsfolder . '/multilingual-permalinks.php' ) && $xili_language->is_permalink ) {
					...
					$field_args = array(
						'option_name' => $this->settings_name,
						'title'     => xl__( 'Activate Permalinks', $this->admin_ui_domain ),
						'type'      => 'checkbox',
						'id'        => 'perma_ok',
						'name'      => 'perma_ok',
		...
		public function xili_display_one_section( $section ) {
			...
			switch ( $section['id'] ) {
				...
				case 'xili_option_section_2':
						/* translators: */
					echo '<p><span class="triangle"></span>' . sprintf( esc_html__( 'Enable (or not) instantiation of the registered menu locations.<br /> After changes saved, <a href="%s" >go to Menus settings</a> and fill menus for each language.', 'xili-language' ), 'nav-menus.php' ) . '</p>';
					break;
				case 'xili_option_section_3':
					/* translators: */
					echo '<p><span class="triangle"></span>' . sprintf( esc_html__( 'Enable (or not) instantiation of the registered sidebars.<br /> After changes saved, <a href="%s" >go to Widget Menus</a> and fill sidebar for each language.', 'xili-language' ), 'widgets.php' ) . '</p>';
					break;
				case 'xili_option_section_4':
					echo '<p><span class="triangle"></span>' . xl_esc_html__( 'Allow (or not) to copy feature or attribute when creating a post from one language to another.', $this->admin_ui_domain ) . '</p>';
					break;
				case 'xili_option_section_5':
					echo '<p><span class="triangle"></span>' . xl_esc_html__( 'Allow (or not) to choose a header image per language.', $this->admin_ui_domain ) . '</p>';
					break;
				case 'xili_option_section_6':
					echo '<p><span class="triangle"></span>' . xl_esc_html__( 'Assignment of header image to a language.', $this->admin_ui_domain ) . '</p>';
					break;
				case 'xili_option_section_7':
					echo '<p><span class="triangle"></span>' . xl_esc_html__( 'Activate Languages (or aliases) in permalinks.', $this->admin_ui_domain ) . '</p>';
					break;
			}
		 ...
		 * one line in section
		 */
		public function xili_display_one_setting( $args ) {
			extract( $args );
		...
		public function show_header_assignment_fields( $headers ) {
			...
			foreach ( $headers as $header_key => $header ) {
				...
				$field_args = array(
					'option_name' => $this->settings_name,
					'title' => $header_desc,
					'desc' => xl__( 'Select language where this header will be displayed...', $this->admin_ui_domain ),
		...
		public function show_one_header_to_select( $header ) {
			extract( $header );
			echo '<div class"' . $class . '">';

			$default_class = ( get_theme_mod( 'header_image' ) == $header_url ) ? ' class= "header-imgs outlined" ' : ' class= "header-imgs" ';
		...
		public function show_language_pop_up_selector( $header ) {
			extract( $header );
		...
		public function xili_validate_settings( $input ) {
			...
			if ( $theme_mod_locations ) { // if new theme not started
				...
				foreach ( $menu_locations_keys as $oneloc ) {
					...
					if ( ( $checked_locations && ! in_array( $oneloc, $checked_locations ) ) || ( array() == $checked_locations && ! isset( $input['nav_menus'] ) ) ) {
						foreach ( $language_slugs_list as $slug ) {
							$one_menu_location = $oneloc . $this->menu_lang_sep . $slug;
			...
			if ( ! isset( $input['no_flags'] ) ) {
				$newinput['no_flags'] = '';
			}
			if ( ! isset( $input['linked_posts'] ) ) {
				$newinput['linked_posts'] = '';
			}
			if ( ! isset( $input['nav_menus'] ) ) {
				$newinput['nav_menus'] = '';
			}
			if ( ! isset( $input['xl_header'] ) ) {
				$newinput['xl_header'] = '';
			}
			if ( ! isset( $input['perma_ok'] ) ) {
				$newinput['perma_ok'] = '';
			}
		...
		public function print_styles_xili_options( $params ) {
			global $wp_version;
		...
		public function print_scripts_xili_options() {
			wp_enqueue_script( 'jquery-ui-accordion' );

		}

		public function xili_theme_options_help_page() {
			...
			$help = '<p>' . sprintf( __( 'Some themes provide customization options that are grouped together on a Theme Options screen. If you change themes, options may change or disappear, as they are theme-specific. Your current theme, %1$s%2$s, provides the following Theme Options:', 'xili-language' ), $this->theme_name, $this->child_suffix ) . '</p>' .
					'<ol>' .
						...
						'<li>' . xl__( '<strong>One Header Image per language</strong>: After adding (and cropping) a series of images in Appearance Header settings page, here you assign a language per image. If not choosen, the default header image (black border) is selected for unknown language.', $this->admin_ui_domain ) . '</li>' .
						/* translators: */
						'<p>' . sprintf( xl__( 'The images must be uploaded (and cropped) in the %1$sheader settings%2$s page.', $this->admin_ui_domain ), '<a href="' . $header_setting_url . '">', '</a>' ) . '</p>' .
						'<li>' . xl__( '<strong>Language in Permalinks</strong>: Reserved for donators. Never forget to refresh permalinks after changing settings !', $this->admin_ui_domain ) . '</li>' .
