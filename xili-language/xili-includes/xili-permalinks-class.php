...
class XL_Permalinks_Rules {
	...
	public $language_slugs_list;
	public $language_xili_settings = array();
	 ...
	 *
	 */
	public function create_default_link( $termlink, $term, $taxonomy ) {
		if ( ! is_object( $term ) ) {
			if ( is_int( $term ) ) {
	 ...
	 *
	 */
	public function language_slugs_list_being_updated() {
		if ( isset( $_POST['language_settings_action'] ) ) {
			// form form first xl settings tab
			$array_alias = array();
	...
	public function type_rewrite_rules( $rules ) {
		...
		foreach ( $rules as $key => $rule ) {
			...
			} elseif ( in_array( $filter, $this->always_rewrite ) || ( $cpts && preg_match( $cpts, $rule ) && ! strpos( $rule, 'name=' ) ) || ( 'rewrite_rules_array' != $filter && $this->options['force_lang'] ) ) {
				if ( isset( $slug ) ) {
					$newrules[ $slug . str_replace( $wp_rewrite->root, '', $key ) ] = str_replace(
						array( '[8]', '[7]', '[6]', '[5]', '[4]', '[3]', '[2]', '[1]', '?' ),
	...
	public function insert_lang_tag_root( $url, $path, $orig_scheme, $blog_id ) {
		...
		if ( false !== strpos( $url, $this->rew_reqtag ) ) {
			return $url; // to avoid twice insertion
		}
		if ( false !== strpos( $url, 'wp-json' ) ) {
			return $url; // to avoid jetpack 5.0 insertion during settings - need TO DO
		}
		...
		} elseif ( preg_match( $pattern_tax, $path ) ) {
			$url = str_replace( $path, '/' . $this->rew_reqtag . $path, $url );

		} elseif ( '' != $path && '/' != substr( $path, 0, 1 ) && false === strpos( $path, $this->rew_reqtag ) ) {
			$url = str_replace( $path, $this->rew_reqtag . '/' . $path, $url );
	 ...
	 *
	 */
	public function insert_lang_tag_4post( $permalink, $post, $leavename ) {
		if ( false !== strpos( $permalink, '?p=' ) ) {
			return $permalink;
		}
		if ( false !== strpos( $permalink, $this->rew_reqtag ) ) {
			return $permalink; // to avoid twice insertion
		}
		if ( false !== strpos( $permalink, 'wp-json' ) ) {
			return $permalink;
		}
	 ...
	 */
	public function insert_lang_shortlink( $shortlink, $id, $context, $allow_slugs ) {
		global $xili_language;
		if ( 0 == $id ) {
	...
	public function insert_lang_4post( $permalink, $post, $leavename ) {
	...
	// added for post_type
	public function insert_lang_4post_type( $permalink, $post, $leavename ) {
		...
		if ( in_array( $post_type, $custompoststype_keys ) && 'enable' == $custompoststype[ $post_type ]['multilingual'] ) {
	...
	// page
	public function insert_lang_4page( $permalink, $post_id ) {
	...
	public function insert_lang_4year( $permalink, $year ) {

		return $this->insert_cur_lang( $permalink );
	}
	public function insert_lang_4month( $permalink, $year, $month ) {

		return $this->insert_cur_lang( $permalink );
	}
	public function insert_lang_4day( $permalink, $year, $month, $day ) {

		return $this->insert_cur_lang( $permalink );
	}
	// used by date, author components
	public function insert_cur_lang( $permalink ) {
		global $xili_language;
		if ( is_admin() ) {
			return $permalink; // in menus builder - not resolve %lang%
		}

		$the_lang = ( false === $xili_language->doing_list_language ) ? $xili_language->lang_slug_qv_trans( the_curlang() ) : $xili_language->lang_slug_qv_trans( $xili_language->doing_list_language ); // by default
	...
	public function insert_lang_taxonomy( $termlink, $term, $taxonomy ) {
		global $xili_language, $xili_tidy_tags, $post;

		if ( is_admin() ) {
			return $termlink; // in menus builder - not resolve %lang%
		}
		$do_replace = false;
		...
		if ( ! $do_replace ) {
			...
			if ( in_array( $taxonomy, $authorized_custom_taxonomies ) ) {
				// test if linked to authorized CPT
	...
	public function insert_lang_pagenum_link( $link ) {
