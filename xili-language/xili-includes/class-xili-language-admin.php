...
class Xili_Language_Admin extends Xili_Language {
	...
	public function post_submit_permalink_option() {
		...
		if ( $post ) {
			...
			if ( '' != $translation_state && false !== strpos( $translation_state, 'initial' ) ) {
				...
				?>
				<p><label for="xl_permalink_option" class="selectit"><input name="xl_permalink_option" type="checkbox" id="xl_permalink_option" value="slug" /> <?php /* translators: */ printf( __( 'Renew slug %s with title', 'xili-language' ), $perma ); ?></label></p>
				<?php
			}
		}
	}
	...
	public function about_screen() {
		...
		<div class="wrap about-wrap">
			...
			<h1>
				<?php
				/* translators: */
				printf( __( 'Welcome to xili-language %s', 'xili-language' ), XILILANGUAGE_VER );
				?>
			</h1>
			...
			<div class="about-text">
				...
				/* translators: */
				printf( __( '%1$s xili-language %2$s is more powerful, stable and secure than ever before. We hope you enjoy using it.', 'xili-language' ), $message, XILILANGUAGE_VER );
				?>
			</div>
	 ...
	 *
	 */
	public function create_library_of_alert_messages() {

		/* translators: */
		$this->admin_messages['alert']['default'] = sprintf( __( 'See %1$sWiki%2$s for more details', 'xili-language' ),'<a href="' . $this->wikilink . '">' ,'</a>' );
		/* translators: */
			...
			<div id="tabs-panel-xlmenulist" class="tabs-panel tabs-panel-active tabs-panel-menu">
				<ul id ="xlmenulist-checklist" class="categorychecklist form-no-clear">
					...
					if ( $nav_menus_wo ) { // now saves slug - lang index as term_id
						...
						foreach ( $listlanguages as $language ) {
							echo '<span class="lang-menu-name" > ' . esc_html__( $language->description, 'xili-language' ) . '</span>'; ?>&nbsp;
		...
		switch ( $action ) {
			...
			case 'sendmail': // 1.8.5
				...
				if ( preg_match( '/^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,6}$/i', $_POST['ccmail'] ) && preg_match( '/^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,6}$/i', get_bloginfo( 'admin_email' ) ) ) {
					...
					$emessage = sprintf( __( 'Thanks for your email. A copy was sent to %1$s (%2$s)', 'xili-language' ), $_POST['ccmail'], $sent );
				} else {
					$msg = 2;
					$emessage = sprintf( __( 'Issue in your email. NOT sent to Cc: %1$s or the return address %2$s is not good !', 'xili-language' ), $_POST['ccmail'], get_bloginfo( 'admin_email' ) );
				}
				break;
		}
	 ...
	 */
	public function available_languages_row() {
		global $wpdb;
		$default = 0;
		/*list of languages*/
		$listlanguages = get_terms_of_groups_lite( $this->langs_group_id, TAXOLANGSGROUP, TAXONAME, 'ASC' );
		...
		foreach ( $listlanguages as $language ) {
			...
			$h = ( isset( $this->xili_settings['lang_features'][ $language->slug ]['hidden'] ) && 'hidden' == $this->xili_settings['lang_features'][ $language->slug ]['hidden'] ) ? '&nbsp;' : '&#10004;';
	...
	public function admin_init() {
		...
		if ( $this->exists_style_ext && ( $this->style_folder != $this->plugin_url ) ) {
			wp_register_style( 'xili_language_stylesheet', $this->style_folder . '/xili-css/xl-style.css' );
		}
	}
			...
			if ( $otherpost && $language->slug != $postlang ) {
				...
				if ( $linepost ) {
					...
					echo '<td>' . $hiddeninput . ' </td><td>' . __( 'not yet translated', 'xili-language' )
						. '&nbsp;&nbsp;' . sprintf( '<a href="%1$s" title="%2$s">' . $creation_edit . '</a>', 'post.php?post=' . $post_id . '&action=edit&xlaction=transcreate&xllang=' . $language->slug, sprintf( esc_attr__( 'For create a linked draft translation in %s', 'xili-language' ), $language->name ) ) . '&nbsp;|&nbsp;' .  $search
						. '</td><td>&nbsp;</td><td>' . $search
						. '&nbsp;'
						. '</td></tr>';
	...
	*/
	 ...
	 *
	 */
	public function download_mo_from_automattic( $locale = 'en_US', $version, $theme_name = '', $upgrade = 0 ) {
	 ...
	 *
	 */
	public function download_mo_from_translate_wordpress( $locale = 'en_US', $version, $theme_name = '', $upgrade = 0 ) {
	...
	public function add_help_text( $contextual_help, $screen_id, $screen ) {
		...
		if ( 'nav-menus' == $screen->id ) { // 2.8.8
			...
			$to_remember =
				...
				'</ul>' .
				'<p>' . sprintf( esc_html__( '%1$sMost recent infos about xili-language trilogy%2$s', 'xili-language' ), '<a href="' . $this->fourteenlink . '" target="_blank">', '</a>' ) . '</p>' .
		...
		if ( 'attachment' == $screen->id ) { // 2.18.1
			$more_infos =
				...
				'</ul>' .
				 '<p>' . sprintf( esc_html__( '%1$sXili-language Plugin Documentation in WP repository%2$s', 'xili-language' ), '<a href="' . $this->repositorylink . '" target="_blank">', '</a>' ) . '</p>' .
				'<p>' . sprintf( esc_html__( '%1$sMost recent infos about xili-language trilogy%2$s', 'xili-language' ), '<a href="' . $this->fourteenlink . '" target="_blank">', '</a>' ) . '</p>';
		...
		if ( in_array( $screen->id, array( 'settings_page_language_page', 'settings_page_language_front_set', 'settings_page_language_expert', 'settings_page_language_files', 'settings_page_author_rules', 'settings_page_language_support' ) ) ) {
			...
			$more_infos =
				...
				'<p><a href="' . $this->devxililink . '/xili-language" target="_blank">' . esc_html__( 'Xili-language Plugin Documentation', 'xili-language' ) . '</a></p>' .
				'<p>' . sprintf( __( '<a href="%s" target="_blank">Xili Wiki Documentation</a>', 'xili-language' ), $this->wikilink ) . '</p>' .
				'<p><a href="' . $this->forumxililink . '" target="_blank">' . esc_html__( 'Support Forums', 'xili-language' ) . '</a></p>' .
