		...
		foreach ( $listlanguages as $language ) {
			...
			if ( ! empty( $pll_description['locale'] ) ) {
				...
				if ( ! is_wp_error( $term_data ) ) {
					...
					$xili_language->xili_settings['lang_features'][ $slug ] = array(
						'charset' => '',
						'hidden' => '',
						'alias' => $xl_alias,
					);
...
function xl_import_previous_pll_aliases( $result ) {
	...
	global $xili_language;
	if ( ( $pll_xl_aliases = get_option( 'xili_language_pll_languages', false ) ) && 2 == $xili_language->xili_settings['pll_cleaned'] ) {
...
function pll_list_messages( $messages, $count = 0 ) {
	...
	if ( ! empty( $xili_language->xili_settings['pll_cleaned'] ) ) {
		...
		if ( 1 == $xili_language->xili_settings['pll_cleaned'] ) {
...
function pll_list_forms_action() {
	...
	if ( ! empty( $xili_language->xili_settings['pll_cleaned'] ) && $xili_language->xili_settings['pll_cleaned'] >= 2 ) {
		?>
		<p><?php esc_html_e( 'This multilingual website was formerly driven by Polylang.', 'xili-language' ); ?></p>
		<?php
		...
		if ( $step < 4 ) {
			printf( '<fieldset id="pll-box" class="box"><legend>%s</legend>', __( 'Polylang importation last actions', 'xili-language' ) );
			wp_nonce_field( $submit_id, 'pll_cleaned' );
			printf( '<label for="%s" class="selectit">&nbsp;', 'pll_action' );
			printf( '<input name="%1$s" id="%1$s" type="checkbox" value="enable">', 'pll_action' );
			printf( '&nbsp;%s</label>', $label );
			printf( '<div class="submit"><input id="%1$s" name="%1$s" type="submit" value="%2$s" /></div>', $submit_id, __( 'Start', 'xili-language' ) );
			echo '</fieldset>';
		} else {
			$plugin_dir = str_replace( 'xili-language/xili-includes', 'polylang', plugin_dir_path( __FILE__ ) );
			$plugin_dir = trailingslashit( $plugin_dir );
